...
class TransferRateFormatter : DefaultFormatter() {
    override fun valueToString(value: Any?): String {
        if (value !is Float)
            throw ParseException("Value is not a float", 0)
        return "%.2f KB/S".format(value).replace(',', '.')
    }
}

class TransferRateFormatterFactory : DefaultFormatterFactory(TransferRateFormatter())

class ReceivedPackagesFormatter : DefaultFormatter() {
    override fun valueToString(value: Any?): String {
        if (value !is Pair<*, *>)
            throw ParseException("Value is not a pair", 0)
        return "${value.first} of ${value.second}"
    }
}

class ReceivedPackagesFormatterFactory : DefaultFormatterFactory(ReceivedPackagesFormatter())
